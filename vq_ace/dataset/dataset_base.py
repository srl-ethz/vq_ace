        ...
        for key, src, key_start, key_end in self._keys_traj:
            ...
            if key_pad_before > 0:
                if self._pad_type == "zero":
                    ...
                    traj_parts.append(torch.repeat_interleave(slice[:1], key_pad_before, dim=0))
                mask_parts.append(torch.zeros(key_pad_before, dtype=torch.bool))

            if key_end >0 and key_start < end-start:
                traj_parts.append(slice[key_start:key_end])
            ...
            if key_pad_after > 0:
                if self._pad_type == "zero":
                    ...
                    traj_parts.append(torch.repeat_interleave(slice[-1:], key_pad_after, dim=0))
