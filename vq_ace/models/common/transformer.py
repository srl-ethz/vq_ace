...
class MIMO_TRANSENCODER(nn.Module):
    """
    ...
    args:
        output_shapes: list of tuples, each tuple contains:
    ...
    def __init__(self, 
        ...
        self.query_tidxs = []
        self.output_shapes = torch.jit.Attribute(output_shapes, # work as a type check
            List[Tuple[List[int], Dict[str, int]]] 
        ).value
        for t_idxs, heads in output_shapes:
    ...
    def forward(self, src, pos_embed, mask, src_tidxs = None):
        ...
        Args:
            src: bs, seq, d_model
            pos_embed (torch.Tensor): positional embeddings: token, d_model
            mask
        ...
        pos_embed = torch.cat([pos_embed.unsqueeze(0).repeat(bs,1,1), torch.zeros_like(cls_embed)], dim=1) # (bs, seq+cls, d_model)
        ...
        for i, (length, heads) in enumerate(self.output_shapes):
            for outhead, outheaddim in heads.items():
                out[outhead] = self.out_heads[outhead](
